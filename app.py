...
LOGO_FILE = Path("katharineum_logo.png")
...
with h1:
    ...
    st.markdown(
        '<h1 class="fund-title">Fundbox 🔎</h1>',
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        Foto hochladen, KI erkennt den Gegenstand, fertig.
        </p>
        """,
        unsafe_allow_html=True
    )
...
with h2:
    ...
    if LOGO_FILE.exists():
        ...
        st.markdown(
            '<div class="school-fallback">K</div>',
            unsafe_allow_html=True
        )

    st.markdown(
        """
        <div class="school-cap">
            KATHARINEUM<br>
            ZU LÜBECK
        </div>
        """,
...
tab = ui.tabs(
    TABS,
    value=st.session_state.get("tab", TABS[0]),
    key="main_tabs"
)
...
if tab == "Stöbern":
    ...
    st.markdown(
        """
        <div class='fund-hero'>
            <h2>Etwas verloren? 👀</h2>
            <p>
                Stöbere durch die Fundstücke oder lass dir per Foto helfen —
                die KI erkennt Kleidung, Flaschen, Schlüssel & Co. automatisch.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    for idx, cls in enumerate(CLASSES):
        ...
        n = len([
            i for i in items
            if i.get("category") == cls
        ])

        with cols[idx % 4]:

            ui.card(
                title=f"{emoji_for(cls)} {cls}",
                description=f"{n} Stück",
                key=f"catcard_{idx}"
            )
    ...
    if not fresh:
        ...
        ui.alert(
            "Noch nichts da",
            "Sobald etwas gefunden wird, erscheint es hier. "
            "Jetzt schon etwas gefunden? Dann melde es!",
            key="empty_alert"
        )
...
# --------------------------------------------------------------------------
    ...
    q = ui.input(
        ...
        key="q",
        placeholder="z. B. Trinkflasche, Schlüssel, blau …",
        type="search"
    )
    ...
    st.caption(
        f"**{len(items)}** Fundstücke im Katalog"
    )
    ...
    if not hits:
        ...
        ui.alert(
            "Nichts gefunden",
            "Anderen Begriff versuchen — oder war es vielleicht "
            "noch gar nicht im Fundbüro? Dann melde den Verlust "
            "im Sekretariat.",
            key="nohit_alert"
        )
    ...
    if rows:
        ...
        ui.alert(
            "Keine Daten",
            "Noch keine Fundstücke eingetragen.",
            key="nodata_alert"
        )
...
# --------------------------------------------------------------------------
    ...
    ui.card(
        ...
        description=(
            "Foto hochladen → KI erkennt den Gegenstand "
            "→ prüfen & speichern."
        ),
        key="howto_card",
    )
    ...
    if not st.session_state.get("rep_bytes"):
        ...
        with p2:
            ...
            if ui.button(
                ...
                with st.spinner(
                    ...
                    ai = (
                        predict_teachable(pil)
                        or heuristic_guess(pil)
                    )
            ...
            if ai:
                ...
                ui.card(
                    title=(
                        f"Erkannt: "
                        f"{ai['label'].capitalize()} "
                        f"{emoji_for(ai['label'])}"
                    ),
                    description=(
                        f"Kategorie „{ai['category']}“ "
                        f"· {ai['engine']}"
                    ),
                    key="ai_card",
                )
                ...
                if ai["confidence"] < 0.5:
                    ...
                    ui.alert(
                        "Unsicher",
                        "Bitte Name und Kategorie unten "
                        "von Hand prüfen.",
                        key="ai_warn"
                    )
        ...
        name = ui.input(
            ...
            key="rep_name",
            placeholder="z. B. Rote Trinkflasche"
        )
        ...
        kat = ui.select(
            ...
            CLASSES,
            index=(
                CLASSES.index(cat_default)
                if cat_default in CLASSES
                else 0
            ),
            key="rep_kat",
        )
        ...
        desc = ui.textarea(
            "Beschreibung",
            key="rep_desc",
            placeholder="Farbe, Marke, Besonderheiten …",
            rows=3
        )

        if ui.button(
            "In die Fundbox legen ✅",
            key="rep_save"
        ):
            ...
            items.insert(
                0,
                {
                    ...
                    "description": (
                        (desc or "").strip()
                        or
                        f"Automatisch erkanntes Objekt ({label})."
                    ),
                    "search_terms": [
                        label.lower(),
                        (kat or "").lower()
    ...
    if st.session_state.get(
        ...
        choice = ui.alert_dialog(
            ...
            "Eingetragen! 🎉",
            (
                f"„{st.session_state.get('done_id') or 'Der Fund'}“ "
                "ist jetzt in der Fundbox. Wie geht's weiter?"
            ),
